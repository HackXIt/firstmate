...
'6. The helper records the exact default session before provisioning, whether it is running or stopped, and verifies the identical record after teardown.' \
'   A missing or changed default session is a hard tripwire failure, never a cleanup warning to ignore.' \
'' \
'Never bypass the helper, even for a read-only lifecycle probe or cleanup after failure.' \
'The `default` session remains protected whether it is running or stopped.')
else
