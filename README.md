...
Launching the `firstmate` helper, or launching a supported harness inside the checkout, instantiates your first mate - and makes you the captain.
...
Each topic stores its private state under `$HOME/.local/share/firstmate/<slug>` by default, starts from this shared checkout, and does not require setting `FM_HOME` in shell startup files.
When Herdr is installed, the launcher creates or uses a topic-specific Herdr session or workspace so multiple firstmate sessions stay visually separate; the initial Herdr tab and pane are named `firstmate`.
