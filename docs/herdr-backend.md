...
Default-on presentation spaces for non-topic homes have a higher floor of Herdr 0.8.0 for the reason given under [Presentation spaces](#presentation-spaces).
...
### Topic workspace and native restart

The supported primary workflow begins with `fm <topic>`.
...
Each new crewmate or scout in a non-topic home is placed in a disposable one-task workspace by default, on Herdr 0.8.0 and newer.
A non-topic home opts out by writing `off` into local gitignored `config/herdr-presentation-spaces`, and forces the projection on by writing `on`.
A topic home created by `fm <topic>` always ignores this visual preference and keeps the flat initial-workspace topology above.
...
The empty file is the historical presence-based opt-in form, so every non-topic home that had already enabled the projection stays enabled with no migration step, and no previously enabled non-topic home can be turned off by the default or by the floor.
A non-topic home that never created the file gains the projection at its next Herdr spawn on a supported release; that flip is deliberate, and it reaches only the Herdr backend because no other runtime backend has a projection path.
...
An unconfigured non-topic home is therefore projected only on a release at or above the 0.8.0 floor, where every workspace-removal primitive preserves focus and that proof stops being load-bearing.
Below the floor that non-topic home uses the ordinary flat per-home layout instead and warns once per home per detected release, naming the running release and the upgrade that restores the projection.
...
- A failed journal publication or projected workspace create stops that spawn instead of falling back flat, so a Herdr create failure surfaces as a spawn failure in every projected non-topic home rather than only in homes that opted in; every earlier degradation on the fresh projected-create path (no session server, contended presentation lock, absent or ambiguous parent) still warns and continues flat.
- Recovery of an existing presentation journal deliberately refuses the spawn when the shared presentation lock is contended rather than falling back flat, and default-on makes that refusal reachable in any non-topic Herdr home.
