...
Calm hides collapsed thinking labels, mid-turn assistant working notes, the transcript rows for Pi's built-in tool names, the `fm_watch_arm_pi` and `fm_branch_outcomes` tool shells, and canonically classified Firstmate operational user rows.
...
The built-in tool-row adapter preserves Pi's execution behavior, and input delivery, ordering, model context, session storage, diagnostics, and `/export` and `/share` operation remain unchanged.
...
The collapsed-thinking, operational-user-row, and tool-execution-row presentation adapters probe the exact Pi API seam they patch when Calm loads.
...
Calm's built-in tool presentation (`bash`, `read`, `edit`, `write`, `grep`, `find`, `ls`) no longer registers same-name tool overrides.
Instead, Calm patches Pi's tool-execution transcript component and hides those rows at render time while Calm is active.
That preserves the active tool definition from Pi itself or from renderer and behavior extensions such as `pi-tool-display`, so Calm does not take ownership of execution, prompt metadata, diff rendering, pending previews, or compact-output behavior.
Rows already on screen before the first toggle hide and restore with the same live Calm presentation as later rows.
Built-in image payloads remain a supported boundary: Calm hides the call and text shell but leaves image rendering visible when Pi has already materialized image components.
...
`.pi/extensions/lib/fm-calm-visibility.ts` owns the visibility policy, `.pi/extensions/fm-calm.ts` owns the tool-execution row adapter, `.pi/extensions/lib/fm-calm-operational-user-layout.ts` owns the zero-height operational-user row adapter, and `.pi/extensions/lib/fm-calm-working-ship.ts` owns the animated working presentation.
