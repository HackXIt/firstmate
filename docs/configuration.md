...
The initial primary and worker launch carries a one-process topic-launch attestation; the bridge accepts it only with the matching canonical home, checkout, named Herdr session, effective state directory, current project, and, for a worker, exact task record.
