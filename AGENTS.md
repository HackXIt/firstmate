...
config/project-context-links  optional project-name -> absolute authoritative-root pointers for projects whose authoritative context lives outside their dispatched repo; LOCAL to each home and gitignored
...
If `bin/fm-project-context.sh <project-name>` prints a configured external-context pointer, read it before scoping, answering, or briefing that project's work.
...
For a GitLab mirror-fork MR whose GitLab project intentionally has no CI, scaffold the ship brief with `--no-mistakes-skip-ci`; GitHub-counterpart sync or check monitoring is separate explicitly authorized work, not part of that GitLab MR validation.
...
The worker reports the PR when CI first becomes green, or when the explicitly selected no-CI GitLab mirror-fork path reaches its skipped-CI ready point, rather than waiting for merge monitoring to finish.

### PR ready, landing, and teardown

For PR-based ship tasks, the ready signal depends on mode: `no-mistakes` reports `done: PR <url> checks green` after CI is green or `done: PR <url> CI skipped` only when the explicit no-CI GitLab mirror-fork brief flag was used, while `direct-PR` reports `done: PR <url>` after opening the PR.
